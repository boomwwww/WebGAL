...
import { createCommonParser } from '../src/lib/commonParser';
...
describe('CommonParser', () => {
  const test1Txt = fs.readFileSync(
    path.join(__dirname, './scene/test1.txt'),
    'utf-8',
  );
  ...
  it('should stringify sentences', () => {
    expect(parser.stringify(parser.parse(test1Txt), { raw: true })).toBe(
      test1Txt,
    );
  });
