...
export const trimCommonSentence = (commonSentence: CommonSentence): CommonSentence => {
  const commandSentence = {
    ...
    attributes: commonSentence.attributes.map(attr => {
      return {
        ...
        value: attr.value === true ? (true as const) : trimQuote(attr.value.trim()).innerValue,
      };
    }),
  };
...
const trimQuote = (str: string): { innerValue: string, quote: string[] } => {
  if (str.startsWith("'") && str.endsWith("'")) {
    ...
    return { innerValue: str.slice(1, -1), quote: ['`', '`'] };
  } else if (str.startsWith('“') && str.endsWith('”')) { // 中文引号
    return { innerValue: str.slice(1, -1), quote: ['“', '”'] };
  } else {
