import type { SentencesParser, Sentence, SentencesParserConfig, Context, State, StateHandler } from "./types";

export const createSentencesParser = (config?: SentencesParserConfig): SentencesParser => {
  return {
...
// 处理转义字段
const handleEscape = (str: string, index: number): { value: string; rawValue: string; next: number } => {
  let value: string, rawValue: string, next: number;
  const config = escapeConfigs.find((config) => str.slice(index + 1).startsWith(config.key));
  if (config) {
...
type EscapeConfig = {
   ...
   */
  handler: (str: string, index: number) => { value: string; rawValue: string; next: number };
};
const escapeConfigs: Array<EscapeConfig> = [
  ...
  },
  {
    ...
    handler: (str: string, index: number) => {
      ...
      if (!cut.startsWith("\\u")) throw new Error("Escape sequence must start with \\u");
  ...
  },
  {
    ...
    handler: (str: string, index: number) => {
      ...
      if (index + 2 > str.length) throw new Error("Incomplete escape sequence");
      const cut = str.slice(index, index + 2);
      if (cut !== "\\\r") throw new Error("Escape sequence must start with \\\r");
      return {
        value: "",
        rawValue: cut,
        next: index + 2,
